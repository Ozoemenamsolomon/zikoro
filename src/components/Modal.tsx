          ...
          try {
            const res = await fetch(`https://api.cloudinary.com/v1_1/kachiozo/image/upload`,
             ...
             if (res.ok) {
                    ...
                    const { data: imageRecord, error } = await supabase.from('eventPhotos').upsert([
                      { 
...

                                  {
                                    selectedFile ? (
                                        <div className='relative'>
                                             <img src={selectedFile} className='object-contain cursor-pointer mx-auto' width={433} height={294} alt="contentImage" />
                                             
                                             <div className="bg-black rounded-full flex opacity-45 items-center justify-center w-8 h-8 absolute left-72 top-4">
                                                <IoClose className="cursor-pointer text-white" onClick={() =>  setSelectedFile(null)}  />
                                              </div>
                                      </div>
