...
const ContactLayout: React.FC<ContactProps> = ({ contacts }) => {
      ...
      <div className='sm:border rounded-lg bg-white'>
        <div className="w-full">
          <div className="w-full sticky top-0 overflow-auto no-scrollbar flex gap-3 items-center px-6 py-2 border-b ">
            {
                contactNav.map(({label,link})=> <Link key={label} href={link} className={`px-4 py-2 rounded   hover:bg-gray-100 duration-300 ${link===pathname ? 'text-basePrimary':''}`}>{label}</Link>)
            }
          </div>
        </div>
            ...
            <div className="w-full flex overflow-auto no-scrollbar gap-1 items-center px-6 py-3 border-b border-slate-200   sticky top-0 bg-white z-20">
                {
                    ...
                    className={`px-3 py-1 rounded text-nowrap  hover:ring-1 hover:ring-gray-200 duration-300 ${query===searchParams.get('query') ? 'bg-blue-600 text-white':''}`}>{label}</button>)
                }
            </div>
