...
const Appointments = ({appointments}:{appointments:AppointmentLink[]}) => {
  const data = [
    { id: 1, name: 'John Doe', time: '10:30 AM – 12:30 PM', category: 'Meeting' },
    { id: 2, name: 'Jane Smith', time: '01:00 PM – 02:00 PM', category: 'Work' },
    { id: 3, name: 'Mike Johnson', time: '03:00 PM – 04:00 PM', category: 'Call' },
    { id: 4, name: 'Anna Brown', time: '05:00 PM – 06:00 PM', category: 'Review' },
  ];
  return (
    <>
      <header className="flex w-full justify-between gap-4 flex-col sm:flex-row pb-10">
        ...
        <div className="rounded-full bg-basePrimary p-0.5">
            ...
            >
              <p>Upcoming appointment</p><ChevronDown size={18}/>
            </button>
        </div>
      </header>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="border-b text-[12px]">
                <th className="py-3 px-4 lex text-start justify-start">Time</th>
                <th className="py-3 px-4 lex text-start justify-start">Name</th>
                <th className="py-3 px-4 lex text-start justify-start">Category</th>
                <th className="py-3 px-4 lex text-start justify-start"></th>
              </tr>
            </thead>
            <tbody>
              {data.map((item, index) => (
                <tr key={item.id} className={`${index % 2 === 0 ? '' : 'bg-purple-50 '} rounded-full `}>
