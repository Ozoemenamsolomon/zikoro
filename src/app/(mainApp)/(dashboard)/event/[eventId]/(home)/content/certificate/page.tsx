...
import { TCertificate } from "@/types/certificate";
...
const Certificates = ({
  ...
  const { certificates, isLoading, getCertificates } = useGetCertificates({ eventId });

  console.log(certificates, "certificates");

  const { createCertificate, isLoading: creatingCertificate } = useCreateCertificate();

  const { saveCertificate, isLoading: certificateIsSaving } = useSaveCertificate();

  const { deleteCertificate, isLoading: deletingCertificate, error } = useDeleteCertificate();

  const createCertificateFn = () =>
    startTransition(async () => {
      try {
        const certificate = await createCertificate();

        if (certificate) {
          router.push(`certificate/edit/${certificate.certificateAlias}`);
        }
      } catch (error) {
        console.log(error);
      }
  ...
  const makeACopy = async (certificate: TCertificate, eventId: string) => {
    ...
    if (newCertificate) {
      const editedCertificate = await saveCertificate({
        certificateAlias: newCertificate.certificateAlias ?? "",
        payload: {
          certificateAlias: newCertificate.certificateAlias ?? "",
          ...payload,
        },
      ...
      if (editedCertificate) {
        router.push(`edit/${editedCertificate.certificateAlias}`);
      }
    }
  };

  const MakeACopy = ({ certificateAlias }: { certificateAlias: string }) => {
    const { events, isLoading } = useGetEvents();
    const [eventId, setEventId] = useState<number>();
    const certificate = certificates?.find(
      ({ certificateAlias: thisCertificateAlias }) => certificateAlias === thisCertificateAlias
    );
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={async (e) => {
                    e.stopPropagation();
                    clsBtnRef.current?.click();
                    await deleteCertificate({ certificateId: certificateAlias });
  ...
  return (
    <div className="flex flex-col gap-2 px-2 py-4">
      <Button
        ...
        disabled={creatingCertificate || pending}
      >
        <svg
          stroke="currentColor"
          fill="currentColor"
      ...
      <div className="grid-cols-4 grid gap-4">
        {certificates?.map(({ previewUrl, certificateAlias, title, created_at, id }) => (
          <div className="relative h-full">
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <ul>
                  <li className="w-full">
                    <MakeACopy certificateAlias={id} />
                  </li>
                  <li className="text-center p-2 hover:bg-gray-100 text-red-700">
                    <Delete certificateAlias={id} />
                  </li>
                </ul>
              </DropdownMenuContent>
            </DropdownMenu>
            <button
              disabled={certificateIsSaving || creatingCertificate || pending}
              className="border border-gray-200 rounded-md relative w-full h-full"
              onClick={() => router.push(`certificate/edit/${certificateAlias}`)}
            >
