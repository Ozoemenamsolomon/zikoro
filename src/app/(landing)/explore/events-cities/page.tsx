...
export default function EventsCities() {
  ...
  return (
    <div className="">
      <Navbar />
      {/* header */}
      <div className="px-1 lg:px-0 max-w-full lg:max-w-6xl mx-auto pb-12 mt-40 lg:mt-48 min-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-5 gap-y-5 lg:gap-y-12 mt-12 lg:mt-24 px-2 lg:px-0 ">
          {data &&
            data.length > 0 &&
            data?.length &&
            (showMore ? data : data.slice(0, 20)).filter((city) => !isEventExpired(city.startDateTime)).map((city, index) => {
